# Pre-PR mode — two-axis local review

Review of the diff between `HEAD` and a fixed point, along two axes reported side by side, never merged:

- **Standards** — does the code conform to this repo's documented coding standards (plus the smell baseline below)?
- **Spec** — does the code faithfully implement the originating issue / PRD / spec?

Both axes run as **parallel sub-agents** so they don't pollute each other's context; this skill aggregates their reports. Nothing is posted to GitHub in this mode.
...
1. Issue references in the commit messages (`#123`, `Closes #45`, GitLab `!67`, etc.) — fetch via the issue-tracker workflow above.
...
- The brief: "Report — per file/hunk where relevant — (a) every place the diff violates a documented standard: cite the standard (file + the rule); and (b) any baseline smell you spot: name it and quote the hunk. Distinguish hard violations from judgement calls — documented-standard breaches can be hard, but baseline smells are always judgement calls, and a documented repo standard overrides the baseline. Skip anything tooling enforces. Under 400 words. Do not invoke `/code-review-multi-axis` (or any other review skill) and do not spawn further agents — perform this review directly."

**Spec sub-agent prompt** — include:

- The diff command and commit list.
- The path or fetched contents of the spec.
- The brief: "Report: (a) requirements the spec asked for that are missing or partial; (b) behaviour in the diff that wasn't asked for (scope creep); (c) requirements that look implemented but where the implementation looks wrong. Quote the spec line for each finding. Under 400 words. Do not invoke `/code-review-multi-axis` (or any other review skill) and do not spawn further agents — perform this review directly."
...
End with a one-line summary: total findings per axis, the worst issue _within each axis_ (if any), and — once the branch has an open PR — the handoff: rerun `/code-review-multi-axis <pr-num>` to walk the findings into a posted GitHub Review. Don't pick a single winner across axes — that's the reranking the separation exists to prevent.
