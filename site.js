/* ===== DATA =====
 * Catalog is loaded at runtime from skills.json / changelog.json (committed to
 * the repo root, served by Pages alongside this file) — the single source of
 * truth, shared with the amittiwari.me site build. Do not re-inline the arrays.
 */
let repoUrl = "https://github.com/amit-t/skills";
let skills = [];
let changes = [];
...
async function loadData() {
  try {
    ...
    if (res.ok) {
      ...
      if (Array.isArray(doc.skills)) skills = doc.skills;
      if (typeof doc.repoUrl === "string" && doc.repoUrl) repoUrl = doc.repoUrl;
    }
  } catch (_) {
    /* leave skills empty; the grid shows an empty-state */
  }
  try {
    const res = await fetch("changelog.json", { cache: "no-cache" });
    if (res.ok) {
      const doc = await res.json();
      if (Array.isArray(doc.changes)) changes = doc.changes;
    }
  } catch (_) {
...
/* ===== CATEGORIES (populated after data loads) ===== */
let categories = ["All"];
...
async function init() {
  await loadData();
  categories = ["All", ...new Set(skills.map((s) => s.category))];
