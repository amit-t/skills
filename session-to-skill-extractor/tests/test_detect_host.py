...
class TestDetect(unittest.TestCase):
    ...
    def test_copilot_env(self):
        self.assertEqual(detect(env={"COPILOT_AGENT": "x"}, home=self._home([]))["host"], "copilot")
    def test_devin_env(self):
        self.assertEqual(detect(env={"DEVIN_SESSION": "x"}, home=self._home([]))["host"], "devin")
    def test_codex_precedes_copilot_when_both_set(self):
        r = detect(env={"CODEX_HOME": "x", "COPILOT_AGENT": "y"}, home=self._home([]))
        self.assertEqual(r["host"], "codex")
