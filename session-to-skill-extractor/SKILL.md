...
`<work_dir>` defaults to `./.session-to-skill` (see `config.example.json`). `--host` must be one of the adapters actually available (`claude-code`, `codex`, `copilot`, `devin`, `generic` — an unknown host exits 2 with the real list in the JSON error). Per-session load failures are collected into the output's `errors` array rather than aborting the run.
