---
name: resume-handoff
description: Pick up where a previous session left off by loading the newest open handoff document written by /handoff. Reads `.claude/handoffs/<timestamp>-<slug>.md` inside the project root, runs an environment preflight (branch, cwd, uncommitted state) against the handoff's frontmatter, then on user confirmation moves the file to `.claude/handoffs/resumed/`, internalises the content, and **asks the user what to do next** — never auto-executes. Triggers include "/resume-handoff", "resume the handoff", "load the handoff", "continue from the last session", "pick up where we left off". Named `resume-handoff` (not `/resume`) because most agents already ship a built-in `/resume` that restores the prior conversation and would shadow this skill.
...
**Why the name is `resume-handoff` and not `resume`.** Many agents (Claude Code, Codex, others) reserve `/resume` for their built-in "restore the previous conversation" command. A skill named `resume` is shadowed in those environments. `/resume-handoff` is unambiguous: it always means *load the handoff document written by `/handoff`*.
...
2. **Resolve the project root.** Use the exact same resolution order as `/handoff`:
...
5. **Handle the argument.**
   ...
   - **A number** (`/resume-handoff 2`) → target index N from the same table (`list` indexes).
...
7. **Preflight check** vs current state. Compute deltas and surface them as warnings (do not block):
   ...
   - **Status sanity** — if `status` is not `open`, refuse to resume that file and explain (it has already been resumed, or the field was hand-edited). Suggest `/resume-handoff list`.
8. **Summarise.** Print:
   ...
   - Count of other unresumed handoffs at top level (e.g. `2 older unresumed handoffs — /resume-handoff list to see them`). Skip the line if count is zero.
...
11. **On n:** leave the file in place with `status: open`. Print `Left as open. Run /resume-handoff list to see all handoffs.` and exit.
